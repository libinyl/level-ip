...
#define arp_dbg(str, hdr)                                                                                              \
    do {                                                                                                               \
        print_debug("arp " str " (hwtype: %hu, protype: %.4hx, "                                                       \
                    ...
                    hdr->hwtype, hdr->protype, hdr->hwsize, hdr->prosize, hdr->opcode);                                \
    } while (0)
...
#define arpdata_dbg(str, data)                                                                                         \
    do {                                                                                                               \
        print_debug("arp data " str " (smac: %.2hhx:%.2hhx:%.2hhx:%.2hhx"                                              \
                    ...
                    data->sip >> 24, data->sip >> 16, data->sip >> 8, data->sip >> 0, data->dmac[0], data->dmac[1],    \
                    data->dmac[2], data->dmac[3], data->dmac[4], data->dmac[5], data->dip >> 24, data->dip >> 16,      \
                    data->dip >> 8, data->dip >> 0);                                                                   \
    } while (0)
...
#define arpcache_dbg(str, entry)                                                                                       \
    do {                                                                                                               \
        print_debug("arp cache " str " (hwtype: %hu, sip: %hhu.%hhu.%hhu.%hhu, "                                       \
                    ...
                    entry->hwtype, entry->sip >> 24, entry->sip >> 16, entry->sip >> 8, entry->sip >> 0,               \
...
static inline struct arp_hdr *arp_hdr(struct sk_buff *skb) { return (struct arp_hdr *)(skb->head + ETH_HDR_LEN); }
